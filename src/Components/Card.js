...
const Card = ({ info }) => {
  ...
  return (
    <div className="w-[250px] relative bg-white shadow-lg mt-[50px] gap-4  rounded-2xl flex flex-col justify-start items-center px-3 py-5">
      <img src={info.image} alt="Product" className="h-[175px] rounded cursor-pointer" />
      <div className="flex flex-col justify-start items-start w-full gap-y-5 font-semibold border-t-2 border-black-custom pt-3">
        <div className="flex justify-between items-center w-full">
          ...
          <span className=" text-green-600 rounded-lg p-1 text-sm">${info.price}</span>
        </div>
        <div className="flex flex-col gap-2">
          <p className="text-black-custom">{`${info.title.substring(0,20)}...`}</p>
        ...
        <div className="flex justify-center items-center gap-3 w-full">
          ...
          >
            <AiFillHeart className={`${saved ? "text-red-600" : "text-gray-400"} text-xl`} />
          </div>
        </div>
      </div>
    </div>
